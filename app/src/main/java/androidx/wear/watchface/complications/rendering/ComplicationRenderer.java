...
class ComplicationRenderer {
    ...
    public void draw(
        ...
        if ( !drawRangedValue(canvas, currentPaintSet)) {
            drawIconOutlindBorder(canvas, currentPaintSet);
        }
    ...
    private boolean drawRangedValue(Canvas canvas, PaintSet paintSet) {
        if (mRangedValueBoundsF.isEmpty() || mComplicationData.getType() != ComplicationData.TYPE_RANGED_VALUE) {
            return false;
        }
        if (DEBUG_MODE) {
        ...
        float total = range - 2 * STROKE_GAP_IN_DEGREES;
        ...
        canvas.drawArc(
                mRangedValueBoundsF,
            startValue + STROKE_GAP_IN_DEGREES / 2,
                inProgressAngle,
                false,
    ...
    private void drawIconOutlindBorder(Canvas canvas, PaintSet paintSet) {
        ...
        Log.d(TAG, "in drawIconOutlindBorder" + bounds);


        int insetAmount = (int) Math.ceil(paintSet.mInProgressPaint.getStrokeWidth());
        bounds.inset(insetAmount, insetAmount);
        Log.d(TAG, "in drawIconOutlindBorder2" + bounds);
        // Draw the progress arc.
        canvas.drawArc(
            bounds,
            -65f,
    ...
    private void drawIcon(Canvas canvas, PaintSet paintSet) {
        ...
        if (icon != null) {
            ...
            Log.d(TAG, "Drawing icon:" + mIconBounds);
            Rect nc = new Rect(mIconBounds);
            nc.top -= 30;
            nc.bottom -= 30;
            drawIconOnCanvas(canvas, nc, icon);
        }
    }
